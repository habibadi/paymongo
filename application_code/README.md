...
- **Soft-Fail Pattern**: Email validation endpoint intentionally returns 500 status, but frontend handles it gracefully
...
- **4 API Endpoints**:
  ...
  3. `POST /api/validate-email` - Email validation (always returns 500 for soft-fail testing)
...
API Endpoints:
  ...
  POST /api/validate-email - Validate email (always returns 500)
Swagger Docs:
